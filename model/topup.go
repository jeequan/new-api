...
func RechargeJeepay(tradeNo string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("Jeepay充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money))
	}
